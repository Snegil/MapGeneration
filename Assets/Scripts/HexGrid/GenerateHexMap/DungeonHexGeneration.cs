...
public class DungeonHexGeneration : MonoBehaviour
{
    [SerializeField]
    ...
    void Setup()
    {
        ...
        // INITIALISE WALKERS
        walkers.Add(new HexWalker(new(0, 0)));
        walkers.Add(new HexWalker(new(0, 0)));
    }
    ...
    void GenerateFloor()
    {
        do
        {
            for (int i = 0; i < walkers.Count; i++)
            {
                ...
                if (hexMap.GetTile(new Vector3(walkers[i].Position.x, 0, walkers[i].Position.y)) != null)
                {
                    chosenTile = hexMap.GetTile(new Vector3(walkers[i].Position.x, 0, walkers[i].Position.y));
                }

                if (chosenTile == null)
                {
                    hexMap.AddTile(i, walkers[i].Position, HexLevelTile.Land);
                }
            }
    ...
    void GenerateHexMap()
    {
        List<HexTile> hexMapTiles = hexMap.GetAllTiles();

        for (int i = 0; i < hexMapTiles.Count; i++)
        {
            Instantiate(tiles[(int)hexMapTiles[i].Tile], hexMapTiles[i].Position, Quaternion.identity);
        }
    }
